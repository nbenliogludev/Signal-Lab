...
## Problem this solves
Developers often add endpoints and forget to wire up metrics, structured logging, or Sentry capture. This hook reminds the agent to verify all three observability signals are present before considering the work done.
...
If any metric is missing, add it now following `observability-conventions.md`.
...
If logging is missing or uses `console.log`, fix it now following `observability-conventions.md`.
...
### 5. Rule cross-check
- [ ] Endpoint follows `stack-constraints.md` (no forbidden libraries introduced)
- [ ] Error handling follows `error-handling.md` (NestJS HTTP exceptions, not raw errors)
